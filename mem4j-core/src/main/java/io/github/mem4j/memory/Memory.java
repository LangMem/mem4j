...
public class Memory {
	...
	private List<MemoryItem> filterBySemanticRelevance(String query, List<MemoryItem> results) {
		...
		if (results.isEmpty() || results.size() <= 3) {
			logger.debug("Skipping semantic filtering - too few results or high threshold already applied");
			return results;
		}
		...
		return results.stream().filter(item -> {
			...
			boolean hasRelevantKeywords = queryKeywords.stream().anyMatch(keyword -> lowerContent.contains(keyword));
